...
"""
SCS_L1L2est.py — Elastic-Net SDF (L2 shrinkage + L1 sparsity) with nested CV and L1×L2 search

Purpose
-------
Estimates SDF coefficients under an elastic-net style penalization while preserving
the exact KNS (2020) structure:
    min_b  (y − X b)' W (y − X b) + λ1 ||b||_1 + λ2 ||b||_2^2
Here, λ2 (“L2pen”) runs on the κ grid inherited from the L2 step, and λ1 is implemented
via a **cardinality stop** in the LARS-EN path (negative `stop` means “keep exactly k
non-zeros”), enabling precise control over sparsity.

High-level flow
---------------
1) Run `L2est` first on the provided data:
   • Supplies κ grid, cross-validated λ2 values (lCV), and baseline results.
   • Provides TRAIN/TEST panels and anomaly names (possibly rotated if requested upstream).

2) Build the L1 grid:
   • If `l1_lambda_min/max/grid_size` are provided, build a focused cardinality grid;
     otherwise use a default log-space-in-cardinality over [1, N].
   • Optional targeting: `target_factors` (with ±`target_window`) restricts search near k.

3) Pre-cache fold moments:
   • Call `cross_validate(..., cache_run=True)` once to store fold moments (X, y, etc.)
     so subsequent λ1×λ2 evaluations reuse the same cached data (fast & deterministic).

4) For each λ2 in lCV:
   • Sweep L1 grid **backwards** so LARS-EN can reuse path computations.
   • For every (λ1, λ2) pair, record OOS objective, its s.e., fold-level values, and (if requested)
     the per-fold coefficients `cv_phi`.

5) Choose the optimal pair:
   • Apply the same optimizer as L2 (`optfunc`) to the (λ1×λ2) surface.
   • If `target_factors` is set, search only in the specified window around k.
   • Compute a small “winner’s-curse” bias correction using the fold maxima.

6) Package results:
   • `optimal_model_L1L2` with objective, κ, chosen L1 cardinality, actual non-zero count,
     bias correction, improvement over L2, and (optionally) averaged coefficients.
   • Store grids, surfaces, and cv_phi for downstream plotting of the L1–L2 map.

Inputs
------
dates, re, market, freq, anomalies : same as L2est (arrays/DataFrames aligned).
parameters : dict (extends L2 options). Notable keys:
  - 'n_jobs'            : parallelism for λ2 loop (joblib; -1 for all cores).
  - 'L1_truncPath'      : keep truncated paths (speed optimization).
  - 'l1_lambda_min/max' : integer cardinalities to bound the L1 grid.
  - 'l1_grid_size'      : number of L1 points when spanning a range.
  - 'target_factors'    : preferred cardinality (exact when combined with min=max=k).
  - 'target_window'     : ± window when targeting.
  - Plot/export flags   : respected but defaulted off inside this routine.

Outputs
-------
Dict extending the L2 results with:
  - 'cv_test_L1L2'      : (nl2 × nl1) OOS objective surface
  - 'cv_test_se_L1L2'   : matching s.e. surface
  - 'cv_phi_L1L2'       : optional per-fold coefficients for each grid point
  - 'L1range'           : L1 cardinality grid (integers)
  - 'optimal_model_L1L2': {
        'n_total_variables','target_sparsity','n_nonzero',
        'kappa','objective','bias','bias_se','objective_corrected',
        'L2_idx','L1_idx','improvement_over_L2', 'coefficients' (if stored)
    }

Numerical guarantees
--------------------
• Uses the same κ grid and cross-validated λ2 values as L2est.
• Evaluations rely on the same contiguous-fold CV machinery and objective definitions.
• L1 cardinality is enforced through the LARS-EN `stop` convention (negative = number of non-zeros),
  matching the Matlab behavior and preserving exact selection at k when `min=max=k`.
• Computation order (including backward L1 sweep and caching) is fixed for reproducibility.

Intended use
------------
Produces the sparse (“dual-penalty”) SDFs used for Table-4 sparse benchmarks and for
L1–L2 contour plots, with speed-ups that do not alter numerical results.
"""
...
def L1L2est(dates, re, market, freq, anomalies, parameters):
    ...
    if est['plot_L1L2map']: _plot_map(x, L1range, cv_test, est)
    return est
...
def _plot_map(x, L1range, cv_test, p):
    import numpy as np
    ...
    # Clip for nicer dynamic range (as in CSR2 family)
    ...
    # ---- KEY FIX: draw lines on the SAME boundaries as contf ----
    ...
    # Log axes with clean decimal ticks
    ...
    if p.get('results_export', False):
        fig.savefig('results_export/elasticnet_contour.png', dpi=300,
                    bbox_inches='tight', facecolor='white', edgecolor='none')
        print("Saved: results_export/elasticnet_contour.png")

    if p.get('show_plot', False): plt.show()
    else: plt.close(fig)
    return fig
